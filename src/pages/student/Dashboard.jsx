    ...
    try {
      ...
      if (projectsResponse.success) {
        ...
        if (userProjects.length > 0) {
          ...
          const projectsWithDeadlines = userProjects.filter(p => p.expectedCompletionDate)
          let daysUntil = 0
          if (projectsWithDeadlines.length > 0) {
            const earliestDeadline = new Date(Math.min(...projectsWithDeadlines.map(p => new Date(p.expectedCompletionDate))))
            const today = new Date()
            daysUntil = Math.ceil((earliestDeadline - today) / (1000 * 60 * 60 * 24))
          }

          setStats(prev => ({
            ...prev,
            projectProgress: Math.round(avgProgress),
            pendingTasks: inProgress,
            daysUntilDeadline: daysUntil > 0 ? daysUntil : 0
          }))
        }
      }

      // Load feedback
      const feedbackResponse = await feedbackAPI.getAllForStudent()
      if (feedbackResponse.success) {
        setRecentFeedback(feedbackResponse.data.slice(0, 3))
        setStats(prev => ({
          ...prev,
          feedbackReceived: feedbackResponse.data.length
        }))
      }
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-primary-600 mt-2">{stats.projectProgress}%</p>
            </div>
          ...
          <div className="mt-4">
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div className="bg-primary-600 h-2 rounded-full" style={{ width: `${stats.projectProgress}%` }}></div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-green-600 mt-2">{stats.feedbackReceived}</p>
            </div>
          ...
          <p className="text-xs text-gray-500 mt-4">Total feedback from supervisor</p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-orange-600 mt-2">{stats.pendingTasks}</p>
            </div>
          ...
          <p className="text-xs text-gray-500 mt-4">Projects in progress</p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-red-600 mt-2">{stats.daysUntilDeadline}</p>
            </div>
          ...
          <p className="text-xs text-gray-500 mt-4">Until submission deadline</p>
        </div>
      </div>
        ...
        {recentFeedback.length > 0 ? (
          <div className="space-y-4">
            {recentFeedback.map((feedback) => (
              <div key={feedback.id} className="border-l-4 border-primary-500 bg-gray-50 p-4 rounded-r-lg">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-4 mt-2">
                      <span className="text-xs text-gray-500">From: {feedback.supervisorName}</span>
                      <span className="text-xs text-gray-500">
