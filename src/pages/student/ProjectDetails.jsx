...
import { projectAPI, feedbackAPI, evaluationAPI } from '../../services/api'
...
const StudentProjectDetails = () => {
  ...
  const loadProjectData = async () => {
    try {
      ...
      // Load feedback
      const feedbackResponse = await feedbackAPI.getByProject(id)
      if (feedbackResponse.success) {
        setFeedbackList(feedbackResponse.data)
      }

      // Load evaluation
      const evaluationResponse = await evaluationAPI.getByProject(id)
      if (evaluationResponse.success && evaluationResponse.data.length > 0) {
        setEvaluation(evaluationResponse.data[0])
      }
  ...
  const handleSubmitProject = async () => {
    if (!window.confirm('Are you sure you want to submit this project for evaluation?')) {
      return
    }

    try {
      setSubmitting(true)
      const response = await projectAPI.submit(id)
      ...
      if (response.success) {
        ...
        alert(response.message || 'Failed to submit project')
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <nav className="flex items-center text-sm text-gray-600">
        <Link to="/student/projects" className="hover:text-primary-600">Projects</Link>
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <div className="flex items-start justify-between mb-4">
          ...
          <span className={`ml-4 px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap ${getStatusColor(project.status)}`}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <p className="text-2xl font-bold text-primary-600 mt-1">{project.progress}%</p>
          </div>
          <div className="bg-gray-50 rounded-lg p-4">
            <p className="text-xs text-gray-500 uppercase">Documents</p>
            <p className="text-2xl font-bold text-gray-900 mt-1">{project.documents?.length || 0}</p>
          </div>
          <div className="bg-gray-50 rounded-lg p-4">
            <p className="text-xs text-gray-500 uppercase">Feedback</p>
            <p className="text-2xl font-bold text-gray-900 mt-1">{feedbackList.length}</p>
          </div>
          <div className="bg-gray-50 rounded-lg p-4">
            <p className="text-xs text-gray-500 uppercase">Status</p>
            <p className="text-lg font-semibold text-gray-900 mt-1 capitalize">{project.status.replace('_', ' ')}</p>
          </div>
        </div>
      </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <p className="mt-1 text-gray-900">{project.supervisor?.name}</p>
                  </div>
                  <div>
                  ...
                  {project.submissionDate && (
                    <div>
                      ...
                      <p className="mt-1 text-gray-900">{new Date(project.submissionDate).toLocaleDateString()}</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-3">
                    <div className="bg-primary-600 h-3 rounded-full transition-all" style={{ width: `${project.progress}%` }}></div>
                  </div>
                </div>
              </div>
            </div>
              ...
              {feedbackList.length > 0 ? (
                <div className="space-y-4">
                  {feedbackList.map((feedback) => (
                    <div 
                      key={feedback.id} 
                      className="border border-gray-200 rounded-lg p-6 hover:border-primary-300 transition-colors cursor-pointer"
                      onClick={() => feedback.status === 'unread' && handleMarkFeedbackRead(feedback.id)}
                    >
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-2">
                            ...
                            <span className={`px-2 py-0.5 rounded text-xs font-medium ${
                              feedback.status === 'unread' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-600'
                            }`}>
                              {feedback.status}
                            </span>
                          </div>
                          <p className="text-sm text-gray-600">
                            From <span className="font-medium">{feedback.supervisorName}</span> • {new Date(feedback.createdAt).toLocaleDateString()}
                          </p>
                        </div>
                        {feedback.rating && (
                ...
                <div className="text-center py-12 bg-gray-50 rounded-lg">
                  ...
                  <p className="mt-2 text-sm text-gray-600">No feedback received yet</p>
                </div>
              )}
            </div>
          )}
              ...
              {evaluation ? (
                <div className="space-y-6">
                  <div className="bg-primary-50 border border-primary-200 rounded-lg p-6">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="text-lg font-semibold text-gray-900">Final Evaluation</h3>
                      <div className="text-right">
                        <p className="text-3xl font-bold text-primary-600">{evaluation.totalScore}/{evaluation.maxTotalScore}</p>
                        <p className="text-sm text-gray-600">Grade: <span className="font-bold text-gray-900">{evaluation.grade}</span></p>
                      </div>
                    </div>
                    <p className="text-sm text-gray-600">
                      Evaluated by {evaluation.evaluatorName} on {new Date(evaluation.evaluatedAt).toLocaleDateString()}
                    </p>
                  </div>

                  <div>
                    <h4 className="font-semibold text-gray-900 mb-4">Evaluation Criteria</h4>
                    <div className="space-y-4">
                  ...
                  {evaluation.generalComment && (
                    <div className="bg-gray-50 rounded-lg p-6">
                      ...
                      <p className="text-gray-700 leading-relaxed">{evaluation.generalComment}</p>
                    </div>
                  )}
                </div>
              ) : (
                <div className="text-center py-12 bg-gray-50 rounded-lg">
                  ...
                  <p className="mt-2 text-sm text-gray-600">Project not yet evaluated</p>
                </div>
              )}
            </div>
          )}
